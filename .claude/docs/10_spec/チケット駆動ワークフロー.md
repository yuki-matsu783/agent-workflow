...
- **スコープ**:
  - 含む: ディレクトリ構成、チケットのスキーマ（フロントマター）、フェーズ×許可マトリクス、PreToolUse / PostToolUse フックの入出力仕様、ガード条件、ワーク境界の判定スクリプト（`work-boundary.sh`）とレビュー状態ファイル、ワーク境界フック（`workflow-boundary.sh`）のブロック条件、テストシナリオ
...
- **チケット運用コマンド（全フェーズ共通）**: `mv` / `git mv`（`wip/10_tickets/` 配下同士の移動に限る）, `git add`（許可パス内に限る）, `git commit`
...
| (d) | (c) と同じ状態で Bash に `gh pr ready` が含まれる（最後のワークもレビューを経ずに ready にしない） | WF011 |

(a)(b) は doing の有無・境界の有無にかかわらず常に適用する。(c)(d) は doing が空のときだけ評価する（doing があれば `workflow-guard.sh` が GitHub 操作と 2 枚目の doing を既に拒否している）。
...
| PreToolUse | `Edit\|Write\|NotebookEdit\|Bash` | `.claude/hooks/workflow-boundary.sh` | レビュー状態ファイルの保護（WF012）とワーク境界の統制（WF011）。doing が空でも動く |
...
| WF011 | ワーク境界違反 | ワーク境界でレビューが完了していない（`review_state != completed`）のに、次のワークのチケットを doing へ移した / `gh pr ready` を実行しようとした（`workflow-boundary.sh`） |
| WF012 | レビュー状態の直接書き換え | `wip/10_tickets/review-state.json` を `work-boundary.sh` 以外の経路（Edit / Write / Bash）で書き換えようとした（`workflow-boundary.sh`。doing の有無を問わず） |
...
**WF011（ワーク境界違反）** — `workflow-boundary.sh` が Bash（`mv` / `git mv` / `gh pr ready`）または `10_doing/*.md` への Write / Edit で返す。doing が無いので「現在のチケット」行の代わりに境界の情報を出す:
...
**WF012（レビュー状態の直接書き換え）** — `workflow-boundary.sh` が Edit / Write / NotebookEdit / Bash で返す:

```
[WF012] レビュー状態の直接書き換え: wip/10_tickets/review-state.json は work-boundary.sh 以外から書き換えできません
...
- WF011 / WF012 は `workflow-boundary.sh`、WF013 / WF014 は `work-boundary.sh` が出す。いずれも `ask` を使わず exit 2 のみ（ヘッドレス実行で「確認できないため拒否」にならない）
...
| TC026g | 境界で `gh pr ready` | Bash / `gh pr ready 13`（todo 空、`review_state: requested`） | exit 2 + WF011 | |
...
`gh` を伴うケース（TC027 / TC028 の非 `--local`）は、`PATH` の先頭にモックの `gh` を置いて固定の JSON を返す方式でテストする（ネットワークに出ない）。
