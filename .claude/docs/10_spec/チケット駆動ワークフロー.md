...
7. done にしたチケットの `type` と `todo` の先頭チケットの `type` を比較する。同じなら 4〜6 を繰り返す。異なる、または `todo` が空なら**ワーク境界**（1つの作業タイプ＝1ワークの完了。`.claude/docs/10_spec/スキル体系.md`「ワーク完了チェックポイント」）
8. ワーク境界では、ワーク完了報告（完了した作業タイプ・チケット一覧・ワーク開始コミットからの差分要約・次の作業タイプ）を行い、呼び出し元（`workflow-issue-mr-driven`）があれば制御を返す。単独実行なら `AskUserQuestion` で承認 / 差し戻しを確認し、承認なら次のワークの 4 へ、差し戻しなら同じ作業タイプの追加チケットを 2〜3 の要領で作って 4 へ
9. 全チケット done になったら、成果物（`wip/20_plans/`、`wip/30_reports/`、コード変更）の一覧をユーザーに報告する

ワーク境界の判定はスキルの手順であり、フック（PreToolUse / PostToolUse）は関与しない。doing が空である間はフックが不活性化する（後述「基本フロー（PreToolUse フック）」2）ため、呼び出し元がワーク境界で行う GitHub 操作（`git push` / `gh`）はフックの変更なしに許可される。
